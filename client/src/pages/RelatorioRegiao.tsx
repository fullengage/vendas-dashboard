...
export default function RelatorioRegiao() {
  ...
  // Pagination
  const totalPages = Math.ceil(sortedEstados.length / itemsPerPage);
  const paginatedEstados = useMemo(() => {
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
    return sortedEstados.slice(startIndex, endIndex);
  }, [sortedEstados, currentPage, itemsPerPage]);

  // Reset to page 1 when filters change
  useMemo(() => {
    setCurrentPage(1);
  }, [anoFilter, vendedorFilter, regiaoFilter, mesFilter, sortField, sortDir]);
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  {sortedEstados.length} estados · Página {currentPage} de {totalPages} · Mostrando {paginatedEstados.length} de {sortedEstados.length}
                </p>
              </div>
          ...
          {totalPages > 1 && (
            <div className="p-4 border-t border-border flex items-center justify-between">
              <div className="text-xs text-muted-foreground">
                Mostrando {((currentPage - 1) * itemsPerPage) + 1} a {Math.min(currentPage * itemsPerPage, sortedEstados.length)} de {sortedEstados.length} estados
              </div>
