...
import { ArrowLeft, Calendar, TrendingUp, TrendingDown, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function Relatorio() {
  ...
  // Pagination
  const totalPages = Math.ceil(growthData.length / itemsPerPage);
  const paginatedData = useMemo(() => {
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
    return growthData.slice(startIndex, endIndex);
  }, [growthData, currentPage, itemsPerPage]);
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-card border border-border rounded-lg overflow-hidden">
          <div className="p-5 border-b border-border">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {growthData.length} meses · Página {currentPage} de {totalPages} · Mostrando {paginatedData.length} de {growthData.length}
            </p>
          </div>
          <div className="border-t border-border overflow-x-auto">
              <table className="w-full text-xs border-collapse">
                ...
                <tbody>
                  {paginatedData.map((r: any, idx: number) => {
          ...
          {totalPages > 1 && (
            <div className="p-4 border-t border-border flex items-center justify-between">
              <div className="text-xs text-muted-foreground">
                Mostrando {((currentPage - 1) * itemsPerPage) + 1} a {Math.min(currentPage * itemsPerPage, growthData.length)} de {growthData.length} meses
              </div>
