...
async function importLeads() {
  ...
  try {
    ...
    for (const record of records) {
      try {
        ...
        if ((inserted + skipped) % 100 === 0) {
          console.log(
            `⏳ Processados: ${inserted + skipped}/${records.length} (Inseridos: ${inserted}, Atualizados: ${skipped})`
          );
        }
      } catch (err) {
    ...
    console.log(`📊 Total de registros processados: ${inserted + skipped}`);
    console.log(`✨ Novos leads inseridos: ${inserted}`);
    console.log(`🔄 Leads atualizados: ${skipped}`);
